<template>
  <div class="proposal-detail">
    ...
    <div class="footer">
      ...
      <div class="bt-send" v-if="viewMode === 'agent'" @click="shareProposal">
        <div><span>转发计划书</span></div>
      </div>
      <div class="bt-call" v-if="viewMode !== 'agent'" @click="callAgent">
    ...
    <!-- 分享箭头弹层 -->
    <md-popup v-model="showShareArrowLayerFlag" position="bottom">
      <div class="share-layer">
        <div><img src="@/assets/imgs/proposal/share-layer-arrow.png"/></div>
        <div><img src="@/assets/imgs/proposal/share-layer-oval.png"/></div>
        <div>
          <div class="bt-share-ensure" @click="showShareArrowLayerFlag = false"><span>我知道了</span></div>
        </div>
      </div>
    </md-popup>
  ...
  export default {
    ...
    data(){
      return{
        ...
        viewMode:'', // 建议书详情的查看模式。取值范围：agent:代理人查看；其它值或无此参数表示“分享查看”
        isSelectorShow:false,// 年龄选择
        selectorValue:30,
        showShareArrowLayerFlag:false,// 是否显示分享引导弹层
        // 汇总信息
        summary:{
          appntLabel:'',
          insuredLabel:'',
    ...
    async created(){
      // 强制刷新，影响IOS微信SDK的初始化
      let proposalDetailRefreshed = localStorage.getItem('proposalDetailRefreshed');
      if(proposalDetailRefreshed !== 'true'){
        localStorage.setItem('proposalDetailRefreshed',true);
      ...
      if(!hasCachedData){
        ...
        this.showCover = this.viewMode !== 'agent';

        try{
          let result = await this.getProposalDetail(this.proposalNo);
          if(result.err)
        ...
        let params = CommonUtils.getUrlCode();
        if(params.shareLayer)
          this.showShareArrowLayerFlag = true;
      }else{
        CommonUtils.closeGlobalMask();
      }
    },
    ...
    methods:{
      ...
      async getProposalDetail(proposalNo, viewMode){
        ...
        if(proposal.appntDTO && proposal.appntDTO.sex){
          if(!proposal.appntDTO.age)
            proposal.appntDTO.age = common.getAgeByBirthday(proposal.appntDTO.birthday);
      ...
      async getProposalDetailForAgent(proposalNo){
        ...
        // 2、获取分享连接
        await this.getProposalDetailForShare(proposalNo,true);
        return {data:res}
      },
      /**
       * 获取分享计划书详情
       * @param proposalNo
       * @param justGetShareUrl true:不做后续接口调用; 其它值：整个方法执行完
       * @return {Promise<{err: *}|{data: *}|{err: string}>}
       */
      async getProposalDetailForShare(proposalNo,justGetShareUrl){
        let url = window.location.href;
        if(url.includes('?'))
          url = url.substring(0,url.indexOf('?'));
        // 分享路径指向 path: '/proposal/proposalnologin'
        url = url.replace('detail','proposalnologin');
        ...
        console.log(`-->计划书分享（签名）--生成shareURL:${JSON.stringify(param)}`);
        let res = await plan.initSign(param);
        console.log(`<--计划书分享（签名）--生成shareURL:${res ? JSON.stringify(res):res}`);
        if(!res)
          return {err:'1、无法获取分享路径'};
        if(res.result !== '0')
          return {err:res.resultMessage};

        // 分解返回的shareUrl查询参数
        let shareUrlMap = {shareUrl:res.shareUrl};
        ...
        if(justGetShareUrl) return null;
        ...
        res = await plan.checkSign(param);
